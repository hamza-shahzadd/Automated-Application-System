...
class Record {
    constructor(studentId, docId, type, date, status) {
...
const firstNames = [
    ...
    "Harper", "Evelyn", "Abigail", "Emily", "Elizabeth", "Mila",
"Ella", "Avery",
...
"Arianna", "Vivian",
    "Kaylee", "Sophie", "Brielle", "Madeline", "Peyton", "Rylee",
"Clara", "Hadley"
  ];
...
while (docIds.length < maxCount) {
  ...
  for (let i = 0; i < 5; i++) {
    if (Math.random() < 0.5) {
      newDocId += String.fromCharCode(Math.floor(Math.random() * 26) +
65); // add a random uppercase letter
    } else {
...
//status array
const statuses = Array.from({length: maxCount}, () => Math.random() <
0.5 ? "pending" : "completed");
...
for(let i=0; i<100; i++){
  ...
  for (let i = 1; i <= maxCount; i++) {
    ...
    records.push(new Record(studentIds[i - 1], docIds[i - 1],
typeArray[i - 1], generatedFirstNames[i - 1],statuses[i - 1]));
  }
...
// generateTable(records);
  ...
  function searchRecord() {
    ...
    for (var i = 0; i < records.length; i++) {
      ...
      // Check if the current record matches the selected dropdownoption and user input
      if ((dropDownOption === "id" && record.studentId === userInput) 
||
          (dropDownOption === "type" && record.type === userInput) ||
          (dropDownOption === "status" && record.status === userInput)) {
            
        // Add the matching record to the new array
        matchingRecords.push(record);
        //if dropdown option is student id , app id or doc id thenbreak the loop
//         if(dropDownOption === "studentId" || dropDownOption ===
// "appId" || dropDownOption === "docId"){
//             break;
//             }
      }
      
    }
    if(matchingRecords.length>0){generateTable(matchingRecords);}
  }